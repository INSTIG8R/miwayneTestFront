...
class MetroGeneralRScheduleTests(unittest.TestCase):
    ...
    @unpack
    def test_ccsv(self, origin, destination, service, effectiveDate,effectiveDateTo,  activity):
        working = self.ccsr.find_schedule(origin=origin, destination=destination, service=service,
                                          effectiveDate=effectiveDate, effectiveDateTo=effectiveDateTo, activity=activity)
